    ...
    public class MapGenerator : MonoBehaviour
    {
        ...
        public void Start()
        {
            ...
            for (int y = -1; y < rows+1; y++)
            {
                for (int x = -1; x < columns+1; x++)
                {
                    if (x == -1 || x == columns || y== -1 || y== rows)
                    {
                        ...
                        GameObject floor = Instantiate(wallTiles[r], new Vector2(x, y), Quaternion.identity);
                        floor.name = $"{x}-{y}";
                    }
                }
            ...
            for (int i  = 0; i < numberOfFood; i++)
            {
                ...
                int randonY = UnityEngine.Random.Range(0, rows);
                Instantiate(foodTiles[0], new Vector2(randomX,randonY), Quaternion.identity);
            }
